...
document.querySelector("#buttonEasy").addEventListener("click", function (): void {
...
let allNumbersOfMatches: number[] = [0, 1, 2, 3, 4, 5, 6, 7];
...
function board(): void {
    ...
    cardsArrayEasy.sort(function (): number {
        return 0.5 - Math.random();
    });

    for (let i: number = 0; i < cardsArrayEasy.length; i++) { 
...
function computerflipCard(): void {
    ...
    if (cardFlip == true) {
        ...
        setTimeout(function (): void {
            firstCard.setAttribute("src", cardsArrayEasy[firstRandom].img);
        },         500);

        setTimeout(function (): void {
            secondCard.setAttribute("src", cardsArrayEasy[secondRandom].img);
        },         1000);

        setTimeout(() => {
            checkForMatchesCpu(cardsArrayEasy[firstRandom], cardsArrayEasy[secondRandom], firstCard, secondCard);
...
function flipCard(event: MouseEvent): void {
    ...
    if (cardFlip == false) {
        ...
        (event.target as HTMLImageElement).setAttribute("src", cardsArrayEasy[cardType].img);
...
function checkForMatchesUser(): void {
    ...
    if (cardPicked[0] === cardPicked[2] && cardPicked[1] != cardPicked[3]) {
        ...
        let usersFirstNumber: string = usersFirstId.slice(-1); 
        ...
        let usersSecondNumber: string = usersSecondId.slice(-1); 
